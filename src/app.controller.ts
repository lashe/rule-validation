...
export class AppController {
  ...
  validate(@Body() data) {
    ...
    if (!data.rule.field) {
      throw new HttpException(response4, HttpStatus.BAD_REQUEST);
    }
    if (!data.rule.condition) {
      throw new HttpException(response5, HttpStatus.BAD_REQUEST);
    }
    if (!data.rule.condition_value) {
      throw new HttpException(response6, HttpStatus.BAD_REQUEST);
    }
